...
const CATEGORIAS = ['Todos', 'Guías', 'Mantenimiento', 'Iluminación', 'Audio', 'Novedades'];
  ...
  template: `
    ...
    @if (destacado(); as f) {
      <a [routerLink]="['/blog', f.slug]" class="card group mt-8 grid gap-6 overflow-hidden md:grid-cols-2">
        <div class="relative -m-4 mb-0 grid aspect-video place-items-center overflow-hidden bg-gradient-to-br from-azul-700 to-navy-900 text-6xl md:m-0 md:aspect-auto md:min-h-64">
          <span class="absolute left-4 top-4 z-10 rounded-full bg-voltaje px-3 py-1 text-xs font-bold text-navy-900">Destacado</span>
          @if (f.imagen) { <img [src]="f.imagen" [alt]="f.titulo" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" /> } @else { 📰 }
        </div>
        <div class="flex flex-col justify-center">
          <div class="flex items-center gap-2 text-xs text-black/50 dark:text-white/50">
            <span class="font-semibold text-azul-500">{{ f.categoria }}</span> · <span>{{ f.publicadoEn | date: 'd MMM y' }}</span>@if (f.lectura) { · <span>{{ f.lectura }}</span> }
          </div>
          ...
          <div class="mt-4 flex items-center gap-3">
            <span class="grid h-10 w-10 place-items-center rounded-full bg-azul-700/10 text-sm font-bold text-azul-700">{{ ini(f.autorNombre) }}</span>
            <div><b class="block text-sm">{{ f.autorNombre }}</b><small class="text-xs text-black/50 dark:text-white/50">{{ f.autorRol }}</small></div>
          </div>
        </div>
      </a>
    }
    ...
    <div class="mt-10 flex flex-wrap gap-2">
      @for (c of categorias; track c) {
        <button type="button" (click)="filtro.set(c)"
                ...
                [class]="filtro() === c ? 'border-azul-700 bg-azul-700 text-white' : 'border-black/15 hover:border-azul-500 dark:border-white/15'">{{ c }}</button>
      }
    </div>
        ...
        @if (loading()) {
          ...
          @for (p of posts(); track p.slug) {
            <a [routerLink]="['/blog', p.slug]" class="card group flex flex-col overflow-hidden transition-all duration-300 hover:-translate-y-1 hover:shadow-lg">
              <div class="relative -m-4 mb-3 grid aspect-video place-items-center overflow-hidden bg-gradient-to-br from-azul-700 to-navy-900 text-4xl">
                @if (p.etiqueta) { <span class="absolute left-3 top-3 z-10 rounded-full bg-voltaje px-2.5 py-0.5 text-xs font-bold text-navy-900">{{ p.etiqueta }}</span> }
                @if (p.imagen) { <img [src]="p.imagen" [alt]="p.titulo" class="h-full w-full object-cover transition-transform duration-500 group-hover:scale-105" /> } @else { 📰 }
              </div>
              <div class="flex flex-1 flex-col">
                <div class="flex items-center gap-2 text-xs text-black/50 dark:text-white/50">
                  <span class="font-semibold text-azul-500">{{ p.categoria }}</span> · <span>{{ p.publicadoEn | date: 'd MMM y' }}</span>
                </div>
      ...
      <aside class="space-y-4">
        ...
        <div class="card">
          ...
          <div class="mt-3 flex flex-wrap gap-2">
            @for (t of temas; track t) { <span class="rounded-full bg-black/5 px-3 py-1 text-xs dark:bg-white/10">{{ t }}</span> }
          </div>
        </div>
...
export class BlogComponent {
  ...
  readonly destacado = signal<Articulo | null>(null);
  readonly temas = ['Baterías', 'LED', 'Alternador', 'Audio', 'Start-Stop', 'Cableado', 'Diagnóstico'];

  readonly ini = iniciales;

  private readonly normales = computed(() => this.articulos().filter((a) => !a.destacado));
  readonly masLeidos = computed(() => this.normales().slice(0, 3));
  ...
  constructor() {
    ...
    this.blog.destacado().subscribe({ next: (d) => this.destacado.set(d), error: () => {} });
  }
}
